...
for subrun in range(1, total + 1):
  ...
  cov = np.load(f"../gm2fr/analysis/results/{dir}/subrun_{subrun}_{total}/transform.npz")["cov"]
  res = source_hist.heights - scan_hist.heights
  chisq = res.transpose() @ la.inv(cov) @ res
  chisq /= len(source_hist.centers)
  lst.append(chisq)
print(lst)
...
ax.plot([0.6, 0.7, 0.8, 0.9, 1.0], lst)
ax.set_xlabel("momentum distribution width [%]")
