...
total = len(next(os.walk('/home/myhwang1999/gm2/gm2mt/results/m2-alg_scan_tnorm'))[1])
...
files = [f"../gm2fr/simulation/data/alg_scan_tnorm_{i}_{total}/simulation.root" for i in range(1, total +1)],
...
for subrun in range(1, total + 1):
  scan_hist = Histogram1D.load(f"../gm2fr/analysis/results/alg_scan_t/subrun_{subrun}_{total}/transform.root", "transform_f")
...
ax.plot(list(np.linspace(24797.5, 24857.5, 7)), lst)
ax.set_xlabel("t_norm [ns]")
